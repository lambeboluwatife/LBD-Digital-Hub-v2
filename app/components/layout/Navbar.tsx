...
export default function Navbar() {
  ...
  return (
    <header className="sticky top-0 w-full z-50 backdrop-blur-xl border-b border-border-tech bg-[#080D0E]/90">
      ...
      {isOpen && (
        <div className="md:hidden fixed inset-x-0 top-[65px] bottom-0 bg-[#080D0E]/95 backdrop-blur-2xl border-t border-[#1B2727] z-50 flex flex-col justify-between p-6 overflow-y-auto animate-in fade-in slide-in-from-top-2 duration-200">
              ...
              {NAV_LINKS.map((link) => {
                ...
                return (
                  <Link
                    ...
                    className={`flex items-center justify-between px-4 py-3 rounded text-base font-medium transition-colors ${
                      isActive
                        ? "bg-[#101718] border border-[#12C887]/40 text-[#12C887] font-semibold"
                        : "text-[#DFE3E4] hover:bg-[#101718] hover:text-[#12C887]"
                    }`}
                  >
                    <span>{link.name}</span>
          ...
          <div className="pt-6 border-t border-[#1B2727] space-y-4">
            <a
              href="mailto:contact@lbddigitalhub.com"
              onClick={() => setIsOpen(false)}
              className="w-full py-3.5 px-4 rounded bg-[#12C887] text-[#080D0E] font-semibold text-sm flex items-center justify-center gap-2 shadow-[0_0_20px_rgba(18,200,135,0.25)]"
            >
              <span>Start a Project</span>
              <ArrowRight className="w-4 h-4" />
            </a>
            <div className="text-center font-mono-code text-xs text-[#8A999A]">
              hello@lbddigitalhub.com • Global Engineering
            </div>
          </div>
        </div>
      )}
